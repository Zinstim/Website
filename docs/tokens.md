...
- **Red acts, but only in the dark.** *(17 Sep, DECIDED — Carbon steel.)* On Night, crimson `#AE4047` leads what a person touches: the primary button (Zinc Wash label, 4.9:1, filling to `#F29BA1` with an Obsidian label at 9.2:1), the product fill, the drop. On Day nothing is crimson but the mark, because there red still means failure (`#B0332B` sits 0.033 from the mark's crimson; on Night the failure colour is `#FF7A6B`, 0.208 away). Four rules: one crimson action per screen, crimson never carries small text (3.35:1 on Obsidian), every error keeps a word, and the reveal stays the only place red is material rather than action.
- **`--lid-silver: #80878D`** — the structure colour: diagonal stripes, plates, instrument marks. Read off the jar lid (L 62%, warm in the render, set here in the cool band). Its shadow is `#494E51`. Silver is what keeps black-and-red from reading as the supplement aisle.
- **The ratio is 90 / 7 / 3** across a whole screen: night and metal, crimson where you act, teal for live states.
